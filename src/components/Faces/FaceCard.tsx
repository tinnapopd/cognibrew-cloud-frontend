...
import { Trash2, User, HardDrive, FileKey } from "lucide-react";
...
export default function FaceCard({ face, onDelete, isDeleting }: FaceCardProps) {
    ...
    <Card className="group relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-primary/5 hover:border-primary/30">
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-sm font-semibold leading-none">{face.username}</p>
              <p className="mt-1 text-xs text-muted-foreground">Enrolled Face</p>
            </div>
          </div>
      ...
      <CardContent className="space-y-3 pb-3">
        ...
        <div className="flex items-start gap-2 rounded-lg bg-muted/50 px-3 py-2">
          ...
          <p className="break-all text-xs text-muted-foreground font-mono">{face.s3_key}</p>
        </div>
        ...
        >
          {showVector ? "Hide" : "Show"} embedding vector ({face.embedding.length}-dim)
        </button>
        {showVector && (
          <pre className="max-h-32 overflow-auto rounded-lg bg-muted/50 p-2 text-[10px] text-muted-foreground leading-relaxed">
            [{face.embedding.slice(0, 20).map((v) => v.toFixed(4)).join(", ")}
